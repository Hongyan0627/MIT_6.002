...
class ClusterSet(object):
    ...
    def mergeClusters(self, c1, c2):
        ...
        if (c1 in self.members and c2 in self.members and (c1.maxLinkageDist(c2) > 0)):
            temp_points = []
            ...
            self.add(Cluster(temp_points,self.pointType))
    ...
    def findClosest(self, linkage):
        ...
        for i in range(len(self.members)-1):
            for j in range(1,len(self.members)):
                temp = linkage(self.members[i],self.members[j])
                if result == None or (temp < result):
                    cluster1 = self.members[i]
                    cluster2 = self.members[j]
                    result = temp
...
def test():
    ...
    print hCluster(points, Cluster.singleLinkageDist, 10, False).toStr()
